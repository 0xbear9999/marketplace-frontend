...
function MarketplaceCollectionContent() {
    const [nfts, setNfts] = useState<Listed[]>([]);
    ...
    useEffect(() => {
        (async () => {
            const mint = searchParams.get('collection');
            const res = await auctionService.collectionNFTs(mint);
            setNfts(res);
        })();
    }, [])
